...
namespace PG.LagCompensation.Testing
{
    ...
    public partial class TestLagCompensation : Node
    {
        ...
        [Export]
        private double summedTimeParametricDeprecatedCached;
        ...
        public void Test()
        {
            ...
            switch (testType)
            {
                case TestType.testAllWithLoop:
                    ...
                    for (int i = 0; i < loopCount; i++)
                    {
                        TestParametric(currentTime - catchUpTime, currentTime, false, 1);
                    }
                    summedTimeParametricDeprecatedCached = Time.GetTicksUsec() * 1e-6 - previousTime;

                    GD.Print("summedTimeParametric (deprecated with 'Cached' suffix) " + summedTimeParametricDeprecatedCached);
