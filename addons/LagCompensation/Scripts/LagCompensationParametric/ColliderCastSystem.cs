...
namespace PG.LagCompensation.Parametric
{
    ...
    public static class ColliderCastSystem
    {
        ...
        public static List<HitColliderCollection> OverlapSphereColllections(bool useCached, Vector3 center, float radius, List<HitColliderCollection> listToModifiy = null)
        {
            ...
            for (int i = 0; i < _simulationObjects.Count; i++)
            {
                Vector3 colCenter = useCached ? _simulationObjects[i].GetCachedPosRot.position : _simulationObjects[i].GetTargetNode.GlobalPosition;
                float colRadius = _simulationObjects[i].GetBoundingSphereRadius;

                // check if the checking sphere and the bounding sphere overlap
                if (center.DistanceTo(colCenter) < radius + colRadius)
                {
                    collections.Add(_simulationObjects[i]);
                }
            }
        ...
        /// <param name="listToModifiy">Will be cleared and filled by this check instead of creating a new list.</param>
        /// <returns>List of overlapping collections. If a list has been passed to this function, this will be a reference to same list.</returns>
        public static List<HitColliderCollection> OverlapCapsuleColllections(bool useCached, Vector3 centerStart, Vector3 direction, float centerDistance, float radius, List<HitColliderCollection> listToModifiy = null)
        {
            List<HitColliderCollection> collections;
            ...
            for (int i = 0; i < _simulationObjects.Count; i++)
            {
                ...
                if (_simulationObjects[i].CheckBoundingSphereDistance(useCached, centerStart, direction, centerDistance, radius))
                {
                    collections.Add(_simulationObjects[i]);
                }
            }
        ...
        /// <param name="listToModifiy">Will be cleared and filled by this check instead of creating a new list.</param>
        /// <returns>List of overlapping collections. If a list has been passed to this function, this will be a reference to same list.</returns>
        public static List<HitColliderCollection> OverlapCapsuleColllections(bool useCached, Vector3 centerStart, Vector3 centerStop, float radius, List<HitColliderCollection> listToModifiy = null)
        {
            float centerDistance = (centerStop - centerStart).Length();
            Vector3 direction = (centerStop - centerStart) / centerDistance;

            return OverlapCapsuleColllections(useCached, centerStart, direction, centerDistance, radius, listToModifiy);
        }
